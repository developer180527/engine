...
> the technical reference for G2–G8.
...
|---|---|---|---|
| **1. Material + shader** | shading model, inputs, blend state | everything else | **P5** — the art-direction surface |
| **2. Render graph** | which passes run, in what order | extraction, visibility, submission | **P4** — the primary control surface |
...
### Phase ids, and why they are prefixed

Three renderer documents run phase sequences, and until 2026-08-28 all three
numbered from 1 with no prefix. **"Phase 4" meant three different things, one of
them already finished:**

| | | "Phase 4" was |
|---|---|---|
| **A** | [`renderer-audit-and-plan.md`](renderer-audit-and-plan.md) | submission efficiency — **done** |
| **P** | this document | the render graph — not started |
| **G** | [`rhi-design.md`](rhi-design.md) | porting the forward pipeline — not started |

`roadmap.md` and `renderer-architecture.md` both cite the **A** sequence
("done (A1)", "finish A5"), so a commit message saying "Phase 4" was ambiguous
between finished work and two unstarted plans. Hence `A`/`P`/`G`.

Two letters are deliberately NOT used. `R` is the audit's **finding** ids
(`R1`–`R20`), which appear throughout these documents. And `D` — the obvious
choice for a *design* document — was tried and rejected: `rhi-design.md` names
**D3D12** fourteen times, so a phase called `D3` sits next to it and reads as a
typo. `G` is for the GPU-driven substrate that document exists to specify.

### How P maps onto G

`rhi-design.md` is the technical reference and keeps its own sequence, because it
orders the work by *dependency* while this document orders it by *when it pays*.
They are not a renaming of each other:

| this document | `rhi-design.md` | note |
|---|---|---|
| P0a | G0a | the same spike |
| P1 | G1 | de-contaminate |
| P2 | — | closing the seam is this document's; G folds it into its §7 step 1 |
| P3 | — | the retained scene is this document's addition, and its largest one |
| P4 | G5 | **reordered.** P4 builds the graph on bgfx, before any RHI; G5 builds it on `rhi`, after |
| P5 | — | materials/shaders as assets is this document's; G has no equivalent phase |
| P6 | G0b | **reordered.** G gates almost everything on the GPU farm; P defers it until the first claim that needs it (§3.1 there) |
| P7 | G2 | `rhi` core |
| P8 | G4, G6, G7, G8 | port, GPU-driven, RT, mesh shaders |

**One gap this mapping exposed: `G3`, the shader toolchain, has no P phase.**
`rhi-design.md` §5 calls it "the hidden 40% of the project" and §10 names a
6-week slip in it as a reason to stop — and P8 cites only "§8 G4–G8", so the
single item both documents agree is the most likely sinkhole is the one the
schedule has no row for. It has to become its own P phase before P7 starts.
Recorded here rather than silently folded into P8, because a 40% risk item
without a row is how it arrives as a surprise.

### The phases

Each must be independently defensible. P0a–P3 need no GPU farm and no new
...
| **P7** | **`rhi` core** — device, queues, timelines, resources, bindless, command lists. **Metal 4 + Vulkan 1.3, designed together. No third backend.** | One triangle on both, under validation layers, zero warnings. |
| **P8** | **Port the forward pipeline, no per-draw uniforms**, then GPU-driven, then RT / mesh shaders. | As `rhi-design.md` §8 G4–G8. |

P6–P8 are `rhi-design.md`'s project (its G2–G8), unchanged except for backend
priority. P0a–P5 are this document's, and every one of them is worth doing even if
the RHI is never built.

**Corrected 2026-08-28 while prefixing the sequences.** This paragraph read
"P5–P8 are rhi-design.md's project / P0a–P4 are this document's", which
put the boundary in the wrong place: P5 (Material + shader assets) is this
programme's own — §2 lists it as where Tier-1 control ships, and `rhi-design.md`
has no equivalent phase. §5's cost table already used the 6–8 boundary, so the two
sections disagreed. They now agree, and the gap that disagreement was hiding is
visible: **P5 has no cost line in §5.** It is left unstated rather than filled in
...
| cost | size |
|---|---|
| P0a–P4 | weeks each; the whole group is months, not a year |
| P6–P8 (`rhi-design.md` §9, its G2–G8) | ~6 months to beat what exists, ~1 year to the full vision |
...
| **No golden-image testing** | still the reason `src/render` is `tier: working`; P6 is what finally makes it possible |

## 6. What would make us stop

- **P0a moves nothing and names no wall.** Then the bottleneck is not where
  this document says, and the answer is incremental extraction and job-system work.
- **P3 lands and extraction is still the frame's cost.** Then the retained
  scene was the wrong mechanism and P7–P8 are unjustified.
- **P6 shows the GPU is nowhere near the limit on discrete hardware.**
...
3. **The three floors, written down as tiers.** `rhi-design.md` §11.3 now carries
   ...
   and `MTLIndirectCommandBuffer` set it — and it has to be a number before P7.
4. **Does vCAD adopt the substrate, or stay on bgfx?** It can stay on bgfx through
   P0a and adopt later. Nothing in this plan requires deciding now, and that
   is intentional.
...
> **Take:** P3 is not "design a retained scene." It is *a sub-allocator, a
> sparse uploader, and an apply point.* That is a far smaller and more testable
> thing than the phrase suggests, and it is why P3 sits before any RHI work.
...
is the part of P3 to budget most time for. Unreal's game-thread/render-thread
...
*months*. P4 introduces the graph while `ForwardPipeline` still runs on bgfx,
and P7–P8 add an RHI path while both still exist. **Three renderers coexisting
...
> resource-registration contract *before* P4 ships, not after the first
> corruption bug. It is an exit criterion on P4 for that reason.
