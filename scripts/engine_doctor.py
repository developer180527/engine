...
# gate.
# ── One file per bug, not one file of bugs ──────────────────────────────────
# The ledger was a single markdown file. It went from 245 lines to 1051 in five
# days — ~28 lines an entry, and entries grow after they are written, so the
# count rose even on days that added none. Extrapolated that is ~5 000 lines by
# October, and the failure it produces is not "the file is big": it is that
# `grep` stops answering the question. A match in a 5 000-line file needs
# `-B5 -A8` to see its entry and can still straddle two of them.
#
# `docs/process/bugs/BUG-NNNN-short-title.md` fixes that by making the FILENAME
# the answer:
#
#     grep -rl "operator new" docs/process/bugs/
#     -> docs/process/bugs/BUG-0028-operator-new-under-promised-alignment.md
#
# Three things fall out for free: every `grep -r` match line carries the id and
# the title with no flags, `git log` over one file is that bug's history alone,
# and two branches adding entries stop colliding on one append region.
#
# NO GENERATED INDEX, deliberately. The directory listing is the index. A
# checked-in index file would have to be regenerated by the same commit that
# adds an entry, which is precisely the shape of BUG-0027.
BUG_DIR = REPO / "docs" / "process" / "bugs"
# The handbook — the rule, the schema, the classes. No entries live here any
# more, so nothing parses it; it is named only so the self-pin check below can
# refuse it, and so this constant does not have to be reinvented by the next
# thing that needs the path.
...
BUG_REQUIRED = ("found", "class", "where", "symptom", "cause", "status")

# ── `status`, and why it had to become a field ──────────────────────────────
# `pinned-by` was required unconditionally, so an entry recording a bug that was
# found but NOT fixed had nowhere to put that. Two entries solved it by writing
# `pinned-by: docs/process/bug-ledger.md` — pointing at the ledger itself. The
# gate only checks the path exists, and that path is the file doing the
# checking, so those entries could never fail it.
#
# The cost was not hypothetical. BUG-0023 sat marked "NOT FIXED" for two days
# after its fix shipped (tests/audio_provider_asan_test.cpp), and nothing could
# notice, because to the gate a self-referential pin looks exactly like a real
# proof. A ledger that reports a gap which has been closed is worse than one
# missing the entry: someone re-does the work.
#
# So the state is explicit. `open` needs no proof and MUST NOT claim one;
# `fixed` requires a real one. Self-referential pins are rejected outright.
BUG_STATES = ("open", "fixed")

_BUG_SELF_PIN = ("docs/process/bug-ledger.md", "docs/process/bugs")
...
def parse_bug_ledger() -> list[dict]:
    """Entries as dicts, one per file in BUG_DIR.

    Format is deliberately stdlib-parseable — this script has no third-party
    dependencies and adding one for a bug list would be a poor trade.

    Sorted by filename, which sorts by id, because BUG-NNNN is zero-padded. That
    is worth one sentence: it means `bugs` output and error ordering stay stable
    as files are added, without the script knowing anything about ordering.
    """
    if not BUG_DIR.is_dir():
        return []
    entries: list[dict] = []
    for path in sorted(BUG_DIR.glob("BUG-*.md")):
        cur: dict | None = None
        for lineno, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            head = _BUG_HEAD.match(raw)
            if head:
                # A second heading in one file would silently hide an entry from
                # every check below, so it is refused rather than merged.
                if cur is not None:
                    cur.setdefault("_extra_heads", []).append(lineno)
                    continue
...
def check_bugs() -> list[Finding]:
    ...
    if not entries:
        return [Finding("warn", str(BUG_DIR.relative_to(REPO)),
                        "no ledger entries found")]

    seen: dict[str, str] = {}
    for e in entries:
        bid = e["id"]
        # Findings name the ENTRY'S OWN FILE, not a line number in a shared one.
        # That is most of the point of the split: the report is now a path you
        # can open, and it stays correct when entries above it change size.
        rel = str(e["file"].relative_to(REPO))

        if e.get("_no_head"):
            f.append(Finding("error", rel,
                             "no `## BUG-NNNN — title` heading; the file is not an entry"))
            continue
        for extra in e.get("_extra_heads", []):
            f.append(Finding("error", rel,
                             f"a second `## BUG-` heading at line {extra} — one bug per "
                             f"file, or everything after the first is invisible to this gate"))

        # The filename carries the id, so the two can disagree — which is what a
        # copy-pasted entry looks like, and it would put a real bug under
        # another bug's name in every grep result.
        if not e["file"].name.startswith(bid + "-"):
            f.append(Finding("error", rel,
                             f"filename does not start with `{bid}-` — the id in the "
                             f"heading and the id in the name must match"))

        if bid in seen:
            f.append(Finding("error", rel,
                             f"{bid} also declared in {seen[bid]} "
        ...
        # ── status, and what each state may claim ───────────────────────────
        st = e.get("status", "")
        pin = e.get("pinned-by", "")
        if st and st not in BUG_STATES:
            f.append(Finding("error", rel,
                             f"{bid}: unknown status `{st}` (want: {'/'.join(BUG_STATES)})"))
        elif st == "fixed" and not pin:
            f.append(Finding("error", rel,
                             f"{bid}: `status: fixed` requires `pinned-by:` — a fix "
                             f"without a failing test is not fixed, it is edited"))
        elif st == "open" and pin and pin.lower() not in ("none", "n/a"):
            f.append(Finding("error", rel,
                             f"{bid}: `status: open` must not claim `pinned-by: {pin}` "
                             f"— use `none`, or mark it fixed"))
        # Pointing at the ledger itself passes an existence check and proves
        # nothing. BUG-0023 hid a shipped fix behind exactly this for two days.
        if any(pin.startswith(s) for s in _BUG_SELF_PIN):
            f.append(Finding("error", rel,
                             f"{bid}: `pinned-by: {pin}` points at the ledger itself, "
                             f"which always exists — name a test, or `status: open`"))
...
def check_docs(docs: list[Doc], strict_missing: bool) -> list[Finding]:
    f: list[Finding] = []
    # ── "Today" is a timezone question, and it broke the gating leg ──────────
    # `verified:` is a DATE with no timezone. A contributor at UTC+5:30 writing
    # today's local date produces a stamp that a UTC runner, for five and a half
    # hours out of every day, reads as the FUTURE — which this treats as a hard
    # error. That is exactly what happened: three docs stamped 2026-08-25 from a
    # machine where it was, failing on a runner where it was still the 24th.
    #
    # So: compare against UTC, and allow one day of slack. The check exists to
    # catch a TYPO — a doc stamped 2027, or a year transposed — not to police
    # which side of midnight UTC someone lives on. A day of tolerance costs the
    # check nothing it was ever able to detect.
...
def render_status(docs: list[Doc]) -> str:
    ...
    out.append("")
    out.append(f"Generated {datetime.now(timezone.utc).date().isoformat()} from "
...
def main() -> int:
    ...
    b.add_argument("--open", dest="open_only", action="store_true",
                   help="only entries still unfixed (`status: open`)")
    ...
    if args.cmd == "bugs":
        ...
        for e in entries:
            ...
            # OPEN is called out in the listing rather than left to be inferred
            # from `pinned-by: none`. An unfixed bug that reads like a fixed one
            # is how BUG-0023 stayed wrong for two days.
            open_ = e.get("status") == "open"
            n_open += open_
            mark = "OPEN " if open_ else ""
            print(f"{e['id']}  {mark}{e.get('class','?'):<10} {e.get('title','')}")
            print(f"          pinned-by: {e.get('pinned-by','(none)')}")
        print(f"\n{len(entries)} bug(s)" + (f", {n_open} OPEN" if n_open else ""))
            ...
            def stable(t: str) -> str:
                ...
                for l in t.splitlines():
                    ...
                    if l.startswith(("- **Stale docs:**",
                                     "- **Contract errors:**",
                                     "- **Unreviewed docs:**")):
                        # All three are live-check results: functions of git
                        # history and of TODAY'S DATE, not of the tree. Contract
                        # errors read 0 locally and 3 on the runner purely
                        # because a date-only `verified:` stamp landed on the
                        # other side of midnight UTC — so the file went "out of
                        # date" without a byte of the tree changing.
                        continue
